...
//
//        SprinkleToken token = service.getUnlinkedSprinkleToken(tokenValue).orElseThrow(RuntimeException::new);
//
...
//        String tokenValue = StringHandleUtil.generateUriSafeRandomString(5);
//
//        Mockito.when(sprinkleTokenRepository.findByValue(tokenValue)).thenReturn(Optional.empty());
//
//        SprinkleToken token = service.getSprinkleTokenWithGenerate(tokenValue);
//
//        log.info("## Sprinkle Token : {}", token);
//
//        Assert.assertEquals(3, token.getValue().length());
//        Mockito.verify(service, Mockito.times(1)).generateSprinkleToken();
//    }
}
